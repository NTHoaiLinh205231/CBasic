...
void func1(item *list, int n){
	char *s = NULL;
	clear_stdin();
	printf("Nhap ma so mat hang: ");
	cgetline(&s, 0, stdin);
 	remove_tail_lf(s);
	int i = check_id(list, n, s);
   	if (i != -1) {
   		printf(" %-10s %-30s %15s %20s\n", "Ma so", "Ten mat hang", "Gia tien", "So luong trong kho");
   		printf(" %-10s %-30s %15g %15d\n", list[i].id, list[i].name, list[i].price, list[i].number);
   	} else {
   		printf("Khong ton tai mat hang can tim.\n");
   	}
    printf("\n");
}
